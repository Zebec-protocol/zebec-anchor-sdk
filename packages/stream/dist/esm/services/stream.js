...
class ZebecStream {
    ...
    _findStreamingAmountSol(walletAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            const [withdrawEscrow,] = yield this._findSolWithdrawEscrowAccount(walletAddress);
            try {
                const streamingAmount = yield this.program.account.solWithdaw.fetch(withdrawEscrow);
    ...
    _findStreamingAmountToken(walletAddress, tokenMintAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            const [withdrawEscrow,] = yield this._findTokenWithdrawEscrowAccount(walletAddress, tokenMintAddress);
            try {
                const streamingAmount = yield this.program.account.tokenWithdraw.fetch(withdrawEscrow);
                return streamingAmount;
            }
            catch (e) {
                return null;
    ...
    updateFeeVault(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [feeVaultDataAddress,] = yield this._findFeeVaultDataAccount(this.feeReceiverAddress);
            this.console.info(`creating fee vault for with ${fee_percentage}%`);
            const anchorTx = yield this.transactionBuilder.execUpdteFeeVault(this.feeReceiverAddress, feeVaultAddress, feeVaultDataAddress, fee_percentage);
            const tx = yield this._makeTxn(anchorTx);
            const signedRawTx = yield this.anchorProvider.wallet.signTransaction(tx);
            this.console.info("transaction after signing: ", signedRawTx);
            try {
...
class ZebecNativeStream extends ZebecStream {
    ...
    updateStream(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const receiverAddress = new web3_js_1.PublicKey(receiver);
            const [withdrawEscrowAccountAddress,] = yield this._findSolWithdrawEscrowAccount(senderAddress);
            const escrowAccountPublicKey = new web3_js_1.PublicKey(escrow);
            const amountInLamports = (0, utils_1.getAmountInLamports)(amount);
            const anchorTx = yield this.transactionBuilder.execStreamUpdateSol(escrowAccountPublicKey, withdrawEscrowAccountAddress, senderAddress, receiverAddress, start_time, end_time, amountInLamports);
            const tx = yield this._makeTxn(anchorTx);
            const signedRawTx = yield this.anchorProvider.wallet.signTransaction(tx);
            this.console.info("transaction after signing: ", signedRawTx);
            try {
    ...
    fetchStreamingAmount(data) {
        return __awaiter(this, void 0, void 0, function* () {
            const { sender } = data;
            const senderAddress = new web3_js_1.PublicKey(sender);
            const response = yield this._findStreamingAmountSol(senderAddress);
            const nostreamingAmount = { 'amount': new anchor_1.BN(0) };
            if (!response) {
...
class ZebecTokenStream extends ZebecStream {
    ...
    updateStream(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const escrowAccountPublicKey = new web3_js_1.PublicKey(escrow);
            const amountInLamports = yield (0, utils_1.getTokenAmountInLamports)(amount, tokenMintAddress, this.program);
            const anchorTx = yield this.transactionBuilder.execUpdateStreamInitToken(escrowAccountPublicKey, withdrawEscrowAccountAddress, senderAddress, receiverAddress, tokenMintAddress, start_time, end_time, amountInLamports);
            const tx = yield this._makeTxn(anchorTx);
            const signedRawTx = yield this.anchorProvider.wallet.signTransaction(tx);
            this.console.info("transaction after signing: ", signedRawTx);
            try {
    ...
    fetchStreamData(escrow) {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield this.program.account.streamToken.fetch(escrow);
            return response;
        });
    }
    fetchStreamingAmount(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const senderAddress = new web3_js_1.PublicKey(sender);
            const tokenMintAddress = new web3_js_1.PublicKey(token_mint_address);
            const response = yield this._findStreamingAmountToken(senderAddress, tokenMintAddress);
            const nostreamingAmount = { 'amount': new anchor_1.BN(0) };
            if (!response) {
                return nostreamingAmount;
            }
